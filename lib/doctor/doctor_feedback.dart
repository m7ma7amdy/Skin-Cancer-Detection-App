...
class _doctorFeedBackState extends State<doctorFeedBack> {
  ...
  CollectionReference doctorFeedBack =
      FirebaseFirestore.instance.collection('doctorFeedBack');
  addFeedBack() async {
  String feedbackText = feedBackText.text.trim();

  if (feedbackText.isNotEmpty) {
    try {
      await doctorFeedBack.add({"feedBackText": feedbackText});
      showDialog(
        context: context,
        builder: (BuildContext context) {
          return AlertDialog(
            title: const Text("Success"),
            content: const Text("Thanks for your feedback Doctor :)"),
            actions: [
              TextButton(
                onPressed: () {
